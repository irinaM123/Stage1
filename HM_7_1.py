...
class Segment:
    def __init__(self, koord1: tuple, koord2: tuple):
        self.koord1 = koord1
        self.koord2 = koord2

    def length(self):
        length_otr = round(((self.koord2[0] - self.koord1[0])**2 + (self.koord2[1] - self.koord1[1])**2)**0.5, 2)
        return length_otr

    def x_axis_intersection(self):
        if (self.koord1[1] * self.koord2[1] <= 0):
            return True
        else:
            return False

    def y_axis_intersection(self):
        if (self.koord1[0] * self.koord2[0] <= 0):
            return True
        else:
            return False
