...
def everything_for_your_cat(cats_data):
    owner_data = {}
    for i in range(0, len(cats_data)):
        key = f'{cats_data[i][2]} {cats_data[i][3]}'
        if key not in owner_data:
            owner_data[key] = f'{cats_data[i][0]}, {cats_data[i][1]}'
        else:
            owner_data[key] = f'{owner_data.get(key)}; {cats_data[i][0]}, {cats_data[i][1]}'
    our_str = ''
    for i in owner_data:
        our_str = our_str + f'{i}: {owner_data[i]}\n'
    return our_str
